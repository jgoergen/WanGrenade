...
NameList::NameList() {
  if (((listLength * nameLength + 6) + 1 > maxSize) && debug) Serial.println("ERROR: EEPROM OVERFLOW!");
}
...
void NameList::add(Mac client, String name) {
  ...
  else if ((clients.getNum(client) < 0) && debug) Serial.println("WARNING: name list is full!");
